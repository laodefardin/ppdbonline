...
// cek apakah username dan password ada di tabel 
  $query = "SELECT * FROM tb_siswa WHERE username = '$username' AND tanggal_lahir = '$password'";
$result = $koneksi->query($query);
$row = $result->num_rows;
  $sql = $koneksi->query("SELECT * FROM tb_siswa WHERE username = '$username'");
$akun = $sql->fetch_assoc();
  ...
  if ($row > 0 ){ // jika data ada
    ...
    $_SESSION["nama"] = $akun["nama_siswa"];
    $_SESSION["no_pendaftaran"] = $akun["no_pendaftaran"];
    $_SESSION["level"] = $akun["level"];
    $_SESSION["id_siswa"] = $akun['id_siswa'];
    $_SESSION['foto'] = $akun['foto'];
    // $_SESSION['nis'] = $akun['nis'];
    // $_SESSION['id_anggota'] = $akun['id_anggota'];
        echo "<script> document.location.href='views/user/index'; </script>";
}else{
...
<body class="hold-transition login-page accent-lightblue"
  style="background: rgb(12, 80, 113);color: rgb(255, 255, 255);min-height: 407.656px;background-position: bottom;background-size: 900px;background-position-x: left;background-repeat: no-repeat;background-image: url(https://ppdbsd.slemankab.go.id/assets/images/hero-illustration.png);">
    ...
    <div class="card">
      <div class="card-body login-card-body">
        ...
        <form action="" method="post">
          ...
          <small class="form-text text-muted">Masukkan Tanggal Lahir Anda</small>
          <div class="input-group mb-3">
            <input id="password" type="date" class="form-control" value="<?= $password ?>" name="password" tabindex="2"
              required="">
        ...
        <div class="social-auth-links text-center mb-3">
          <a href="index" class="btn btn-default btn-sm btn-flat pull-right"><i class="fa fa-home"></i>
            Kembali Beranda
          </a>
          <a href="login-admin" class="btn btn-default btn-sm btn-flat pull-left"><i
              class="fa fa-user-secret"></i> Login Admin</a>
          <br>
        </div>
        <!-- <p class="mb-0">
          Belum punya akun? <a href="daftar-siswa" class="text-center">Login Admin</a>
        </p> -->
        <!-- <p class="mb-1">
